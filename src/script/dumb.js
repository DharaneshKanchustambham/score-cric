...
async function cricket() {
    let cricketMatches = await axios.get("https://api.cricapi.com/v1/cricScore?apikey=e46af47b-b33e-4979-b894-85deb8875010");
   ...
   data.map(matches => {
        let team1_score="";
        let team2_score="";
        if (matches.status !=="Match not started") {
             team1_score = `${matches.t1} ${matches.t1s} `;
             team2_score = `${matches.t2} ${matches.t2s}`;
        }
        else {
            team1_score = `${matches.t1} 0/0(0)`;
            team2_score = `${matches.t2} 0/0(0)`;
        }

        let status = matches.status
        if(status ==="There is no scorecard available for this match."){
            status = "Match Abadoned";
        }
        let team1_img = matches.t1img
        let team2_img = matches.t2img
        let matchName = matches.series;
        ...
        function getSubstringAfterT(dateTimeString) {
            ...
            if (indexOfT !== -1 && indexOfT < dateTimeString.length - 1) {
                ...
                let substringBeforeT = dateTimeString.substring(0,indexOfT)
                const substringAfterT = dateTimeString.substring(indexOfT + 1);
                return [substringBeforeT,substringAfterT];
            } else {
        ...
        let timeVal = substringAfterT[1]
        let dateVal = substringAfterT[0]
